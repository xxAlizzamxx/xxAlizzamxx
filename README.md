<h1 align="center">Hola 👋, soy Alanis Zambrano</h1>
<h3 align="center">Desarrolladora en formación en IA | Entusiasta de Ciberseguridad | Construyendo tecnología con impacto 🚀</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=TUUSUARIO&label=Visitas%20al%20perfil&color=0e75b6&style=flat" alt="views" />
</p>

---

## 🧠 Perfil Profesional

Soy estudiante de Ingeniería de Sistemas (5° semestre) enfocada en el desarrollo de soluciones tecnológicas con impacto en el mundo real. Me interesan especialmente la Inteligencia Artificial, la ciberseguridad y el desarrollo de software orientado a resolver problemas críticos.

Tengo experiencia creando aplicaciones funcionales, participando en entornos competitivos de tecnología y aplicando metodologías ágiles en proyectos colaborativos.

---

## 🏆 Logros

- 🥇 Ganadora del **VibeCoding Challenge** en el Tech Caribe Fest (Cooweb)
- 🚀 Reconocida por el desarrollo de soluciones tecnológicas innovadoras con impacto social
- 🤝 Experiencia trabajando en equipo bajo presión en entornos tecnológicos

---

## 📱 Proyecto Destacado — Vidalink

**Vidalink** es una aplicación móvil de respuesta a emergencias diseñada para asistir a personas en situaciones críticas antes de la llegada de atención médica profesional.

### 🔹 Funcionalidades principales:
- 📍 Geolocalización en tiempo real mediante integración con mapas
- 🚨 Sistema de alertas que conecta con voluntarios cercanos con conocimientos en primeros auxilios
- 💬 Chat en tiempo real con el voluntario
- 🤖 Asistente con IA para orientación en primeros auxilios
- 🏥 Detección automática del hospital más cercano

### 🔹 Tecnologías utilizadas:
- Desarrollo en **Android Studio (Java)**
- Backend con **Firebase**
- Integración con **APIs de geolocalización y mapas**
- Base de datos en tiempo real

### 🔹 Estado del proyecto:
- Prototipo funcional en fase de pruebas
- Alto potencial de escalabilidad e impacto social

---

## 💻 Habilidades Técnicas

### 🔹 Lenguajes de Programación
- Java
- JavaScript

### 🔹 Desarrollo Frontend
- HTML
- CSS
- Vue.js

### 🔹 Backend y Herramientas
- Firebase
- APIs REST

### 🔹 Bases de Datos
- MySQL (Oracle)

### 🔹 Herramientas de Desarrollo y Gestión
- Git & GitHub
- Jira
- ClickUp
- Kanban

---

## 🔐 Ciberseguridad

- 🛡 Certificada en **Ciberseguridad Básica e Intermedia** por MinTIC
- 🧪 Más de **30 laboratorios prácticos realizados**

### 🔹 Conocimientos en:
- Seguridad web
- Fundamentos de redes
- Introducción al pentesting
- Buenas prácticas de seguridad

---

## 📊 Gestión de Proyectos

- Metodologías ágiles (Scrum)
- Organización y gestión de tareas
- Trabajo colaborativo en equipos tecnológicos
- Uso de herramientas como Jira, ClickUp y tableros Kanban

---

## 🤖 Inteligencia Artificial (En formación)

- Actualmente en proceso de aprendizaje en IA
- Explorando aplicaciones prácticas en soluciones reales
- Interesada en integrar IA en productos tecnológicos

---

## 🌎 Información Adicional

- Nivel de inglés: **C1 (Avanzado)**
- Enfoque en desarrollo de soluciones escalables
- Interés en tecnología con impacto social

---

## 📫 Contacto

- 💼 LinkedIn: https://linkedin.com/in/alaniszambrano
- 📧 Email: alaniszambrano01@gmail.com 

---
## 🌐 Redes

<p align="center">
  <a href="https://www.instagram.com/xxalizzamxx" target="_blank">
    <img src="https://img.icons8.com/fluency/48/instagram-new.png" width="40" />
  </a>
  
  <a href="https://x.com/XxalizzamxX" target="_blank">
    <img src="https://img.icons8.com/fluency/48/twitterx.png" width="40" />
  </a>

  <a href="https://linkedin.com/in/TUUSUARIO" target="_blank">
    <img src="https://img.icons8.com/fluency/48/linkedin.png" width="40" />
  </a>
</p>
⭐ *"La tecnología es más poderosa cuando resuelve problemas reales."*
